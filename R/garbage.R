...
getRunTable = function(run.tag = "mlrRandomBot", excl.run.ids = NULL, local.db = NULL) {
  if(is.null(local.db)){
    numRuns = 200000 #FIXME: fix this once OpenML offers new solution
    results = do.call("rbind", 
      lapply(0:floor(numRuns/10000), function(i) {
        return(listOMLRuns(tag = run.tag, limit = 10000, offset = (10000 * i) + 1, uploader.id = 2702)) # xgboost, OpenML_bot
      })
    )
